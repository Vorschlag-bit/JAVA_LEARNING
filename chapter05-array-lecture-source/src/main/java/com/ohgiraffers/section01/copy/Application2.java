...
public class Application2 {
    public static void main(String[] args) {
        ...
        *   1. for문을 통한 동일한 인덱스 값을 일일이 복사
        *   2. Object의 clone()을 이용한 복사(사용 빈도 높)
        ...
        *   4. Arrays의 copyOf()을 이용한 복사 */
        ...
        /* 목차. 1. for문 활용 */
        int[] copyArr1 = new int[originArr.length];       // new = heap에 새로운 배열을 생성한다는 의미 = 주소값이 무조건 다르다
        for (int i = 0; i < copyArr1.length; i++) {
            copyArr1[i] = originArr[i];
        }
        print(copyArr1, "for문을 활용한 사본");

        /* 목차. 2. clone()을 활용 */                      // 배열에 clone() 메서드를 사용하면 마찬가지로 new를 통해 새로운 배열이 할당된다
        int[] copyArr2 = originArr.clone();
        print(copyArr2, "clone()을 활용한 사본");

        /* 목차. 3. arraycopy()를 이용한 배열 */
        int[] copyArr3 = new int[originArr.length + 3];
        System.arraycopy(originArr, 0, copyArr3, 3, 3);  // 원본의 0번 인덱스부터 3개 씩 잘라서 카피의 3번 인덱스부터 붙여라
        print(copyArr3, "arraycopy()을 활용한 사본");
